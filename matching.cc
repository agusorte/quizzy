...
float similarity(const std::array<std::array<float, kPatchSize>, kPatchSize>& ref,
                const std::array<std::array<float, kPatchSize>, kPatchSize>&  patch){

  //We will implement the metric difference of squares pixel intensities
  // metric is gonna be the squared difference 
  ...
  for (size_t i = 0; i < kPatchSize; i++){
    for (size_t j = 0; j < kPatchSize; j++){
      square_diff += (ref[i][j] - patch[i][j])*(ref[i][j] - patch[i][j]);
    }
  }
